...
{synopt:{opt pre(#)}}{it:Large-N only.} Specifies the number of pre-treatment periods used for averaging or detrending.{break} 
By default, all available pre-treatment periods are used. {break}
{cmd:pre(1)} uses only the last pre-treatment period for demeaning-based transformations.{p_end}
